...
*   **Objective:** Build a minimal Swift macOS app that replicates the Python PoC using CoreML on the Apple Neural Engine.
    ...
    10. **Validate:** Test the app and use the automated comparison to measure its output against the official **Decoder_HAR Golden Reference**. Primary success criterion is achieving comparable metrics (e.g., Mel MSE, Waveform MSE) to the Python decoder-only PoC, with a long-term goal of matching the HAR standard. Ensure it's running on the ANE.
