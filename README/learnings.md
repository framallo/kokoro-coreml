...
The fundamental challenge was the model's heavy reliance on PyTorch operations that are incompatible with Core ML's requirement for a static or predictably dynamic computation graph.

- **`torch.repeat_interleave`**: This was the primary blocker. The model creates an alignment matrix whose shape depends on the *values* inside the predicted duration tensor (`pred_dur`). This is impossible to represent in a static graph, as the tensor shapes are dependent on the model's own output at runtime.
...
- **Dynamic Tensor Creation**: Operations like `torch.full((1, input.shape[1]), ...)` failed because their shapes were derived from other dynamic tensors.

## 2. Architectural Evolution: From Failure to Success

A direct, one-to-one conversion was not feasible. Our journey involved two major architectural phases.

### Attempt #1: The Two-Stage, Bucketed Architecture (FAILURE)

Our first approach was to split the model into two parts: a dynamic `DurationModel` and several fixed-size `SynthesizerModel` buckets (3s, 5s, etc.).

- **The Theory**: The `DurationModel` would handle the dynamic parts (LSTMs, duration prediction), and its output would be padded by the client to fit into a fixed-size `SynthesizerModel`. This would hide the dynamic shapes from the synthesizer.
- **The Reality**: This approach was the root cause of our most critical bugs. Splitting the atomic alignment operation across two models created a fragile interface that led to:
  - **Tensor Shape Mismatches**: The slightest inconsistency in `trace_length` between the two models resulted in garbled audio.
  - **Performance Fallbacks**: The complex interface was difficult for Core ML to optimize.
  - **Malformed Artifacts**: The complexity led to producing invalid `.mlpackage` files that would instantly crash Xcode on project open.

**Conclusion**: This architecture is deprecated. The dividing line between the two models was arbitrary and created more problems than it solved.

### Attempt #2: The "Decoder-Only" Single-Stage Architecture (SUCCESS)

Given the failures of the two-stage pipeline, we radically simplified the architecture by moving the dynamic logic to the client.

- **CPU (Swift):** All dynamic, sequential logic is now handled in Swift. This includes building the alignment matrix (`pred_aln_trg`) from the predicted durations. This logic is transparent, easy to debug, and fast enough on the CPU.
- **ANE (Core ML):** The heavy, parallelizable work of audio synthesis is handled by a single, simple "decoder-only" Core ML model that accepts the pre-aligned features. This model has fixed, static shapes, making it a perfect candidate for ANE optimization.

**The lesson**: The correct dividing line is between the CPU and the ANE, not between two arbitrary model stages. Offloading small, complex, but sequential operations to the CPU is a powerful strategy that unlocks the ANE for the 99% of the work that it's actually good at.

## 3. CoreML Export Deep Dive: Challenges & Solutions

This section details the most difficult technical hurdles encountered during the model export process.

### Challenge: Memory Exhaustion During Synthesizer Export

Exporting the synthesizer model consistently failed with the process being killed by the OS, even on a 64GB RAM machine.

- **Root Cause**: The alignment matrix (`pred_aln_trg`) and subsequent `einsum` operations created massive intermediate tensors. Memory usage scaled exponentially with `trace_length` (the number of input tokens) and `frames` (the length of the audio).
  - Original: `trace_length=128`, 5s frames=120k → `[128, 120000]` matrix = ~60MB
  - Peak Memory: **>8GB**, causing the OOM kill.
- **The Fix**: Drastically reduce `trace_length`. By using the `--debug` flag in `export_synthesizers.py` or manually setting `trace_length=16` in the exporter, we achieved a massive reduction.
  - Fixed: `trace_length=16`, 5s frames=120k → `[16, 120000]` matrix = ~7.7MB (**87% reduction**)
  - Peak Memory: **~4GB**, allowing the export to succeed.
- **Working Commands**:
  ```bash
  # Ensure you are in the kokoro-coreml directory and the venv is active
  ulimit -s 65520 # Increase stack size
  ...
  python examples/export_coreml.py --output_dir ../coreml --duration_only --trace_length 16
  ```

### Challenge: The "tile reps ≥ 1" Saga

The most persistent error during compilation was `Validation error... All values of reps must be at least 1`.

- **Root Cause**: This was a complex issue with multiple contributing factors:
  1.  **PyTorch `expand` -> CoreML `tile`**: PyTorch's `style.expand(...)` operation, used to smear a style vector across a sequence, was converted into a `tile` operation in CoreML. The number of repetitions (`reps`) was derived from the input sequence length.
  2.  **Lost Shape Constraints**: When using `ct.RangeDim` to allow dynamic sequence lengths, the `min=1` constraint would get lost during the MIL graph conversion. The compiler couldn't prove that the sequence length would always be at least 1, so it failed validation.
  3.  **Trace-Time Dead Code Elimination**: A Python-level guard like `if torch.any(zero_len): ...` was evaluated during tracing with a valid input, constant-folded to `False`, and completely eliminated from the exported graph, offering no protection at runtime.
- **The Fix**: The most reliable solution was to switch from `ct.RangeDim` to `ct.EnumeratedShapes` for the `input_ids`. This provides the compiler with a discrete set of known, valid lengths, allowing it to successfully validate the graph.
  ```python
  # In the duration model exporter
  inputs=[
      ct.TensorType(name="input_ids", 
                    shape=ct.EnumeratedShapes([(1, 16), (1, 32), (1, 64), (1, 96), (1, 128)]), 
                    dtype=np.int32),
      ct.TensorType(name="attention_mask", shape=(1, 128), dtype=np.int32), # Must be fixed size
      # ... other inputs
  ]
  ```
- **Key Lesson**: Only one input can use `EnumeratedShapes`. The rest, like `attention_mask` in this case, must have a fixed shape. The client application is then responsible for padding/truncating inputs to match one of the enumerated shapes.

### Challenge: BNNS Crashes and Model Mismatches

Even with correct shapes, we observed runtime crashes inside Apple's `BNNS` and `Espresso` frameworks, particularly with the full synthesizer model.

- **Root Cause**: The full synthesizer graph contained operations that were unstable on the Neural Engine's BNNS path, especially with larger bucket sizes (≥ 5s).
- **Workaround: Decoder-Only Model**: This was the motivation for the decoder-only architecture. By exporting only the final, stable part of the model, we created a smaller graph that completely avoided the problematic operations, eliminating the BNNS crashes.
- **Related Issue: HAR vs. Synthesizer**: Early in development, we had a `KokoroDecoder_HAR.mlpackage` model in the app bundle. This model expects completely different inputs (`har_spec`, `f0_curve`, etc.) than our duration-based pipeline provides (`d`, `t_en`, etc.). This caused "Feature not specified" errors at runtime.
- **The Fix**: Ensure model architecture matches the pipeline. We removed the HAR model and standardized on a single pipeline that uses a `DurationModel` (in Swift) and a `Decoder-Only` model (in Core ML). Always verify the `SHAPE CONTRACT CHECK` logs to ensure the inputs you provide match what the model expects.

## 4. The Solution: A Two-Stage, Bucketed Architecture (Attempt #1)
...
## 4. Swift Integration & Runtime Challenges

Getting the exported models to run correctly in the Swift application presented its own set of challenges.

### Challenge: The Silent Tokenizer Failure

The most insidious bug: models loaded, preflight passed, but TTS produced only beeps.

- **Root Cause**: The application was failing to load the `config.json` file containing the vocabulary, but this failure was silent. The `KokoroTokenizer` would initialize with an empty vocab (`vocab.count = 0`), causing every tokenization attempt to produce no IDs, which in turn caused the synthesis pipeline to fail gracefully and play a beep. The `config.json` was missing from the app bundle due to an inconsistent SPM resource copying behavior.
- **The Fix**: A multi-layered solution:
  1.  **Resilient Loading**: The tokenizer now attempts multiple ways to find `config.json` in the bundle.
  2.  **Embedded Fallback**: If all loading attempts fail, it uses a hardcoded, minimal vocabulary of basic English characters. This ensures the app never has a completely empty vocab.
  3.  **Python Bridge**: For highest quality, the app uses a Python bridge to call the original Kokoro phonemizer. This is controlled by `UserDefaults` and has a timeout to prevent hangs. The `dev_tokenize.py` script was hardened to only print JSON to `stdout` for robust parsing.
- **Lesson**: Never assume bundle resources load successfully. Implement fallbacks and diagnostics for all critical resource loading.

### Challenge: Xcode Bundling and `.mlpackage` vs. `.mlmodelc`

At runtime, the app would sometimes load an old, broken version of a model, even after we had replaced it with a new `.mlpackage`.

- **Root Cause**: Xcode's build system has a behavior where if it finds a standalone `.mlmodel` file in your resources, it will auto-compile it into a `.mlmodelc` directory in the final app bundle. This compiled version can take precedence over the `.mlpackage` directory you intended to use, leading to confusing, hard-to-debug errors. A malformed `.mlpackage` could also cause Xcode to crash on open.
- **The Fix**:
  - Keep your source control clean. Do not commit stray `.mlpackage` or `.mlmodel` files.
  - Ensure only the intended `.mlpackage` directories are included in the "Copy Bundle Resources" build phase.
  - Delete any standalone `.mlmodel` source files from your project.
  - When in doubt, perform a clean build (`Shift+Cmd+K`) and delete the `DerivedData` directory.

### Challenge: Debugging Artifacts and Runtime Validation

To debug shape mismatches and quality regressions, we needed a reliable way to inspect the data at each step of the pipeline.

- **The Solution**: We implemented a robust on-disk artifact pipeline in Swift. For each synthesis run, the app can save:
  - `ids.json`: The exact token IDs fed to the model.
  - `chunk_..._asr.csv`: The acoustic features (`asr`) spectrogram going into the decoder.
  - `chunk_..._tts.wav`: The final waveform produced.
- **Benefit**: This allows for direct, apples-to-apples comparison between the Swift pipeline's output and a "golden reference" generated by the Python code, which is invaluable for debugging numerical parity.

## 5. Operational Playbook

### Environment Setup (Critical)

Use a dedicated, clean Python virtual environment with pinned dependencies to ensure reproducible exports.
...
### Essential Dev Toggles (UserDefaults)

Use these toggles to isolate different parts of the system for faster debugging.

```bash
# Force CoreML to CPU for deterministic behavior and better error messages
defaults write com.transcendence.talktome com.talktome.coreml.computeUnits cpuOnly

# Enable the Python tokenizer bridge for high-quality phonemes
defaults write com.transcendence.talktome com.talktome.dev.usePythonTokenizer -bool YES
defaults write com.transcendence.talktome com.talktome.dev.tokenizerScript -string "/path/to/kokoro-coreml/dev_tokenize.py"
defaults write com.transcendence.talktome com.talktome.dev.configPath -string "/path/to/kokoro-coreml/checkpoints/config.json"
defaults write com.transcendence.talktome com.talktome.dev.tokenizerPython -string "/path/to/your/.venv-coreml/bin/python"

# Enable saving debug artifacts
defaults write com.transcendence.talktome com.talktome.coreml.dumpSpectrograms -bool YES
defaults write com.transcendence.talktome com.talktome.coreml.dumpWaveforms -bool YES
```

## 6. Performance Baselines

These baselines were captured on an M2 Ultra with 64 GB RAM.

- **CPU/PyTorch Fallback**:
  - "The quick brown fox …": 1.768 s compute for 3.250 s audio → **RTF ≈ 0.54×**
- **GPU (MPS) PyTorch**: Slower than CPU due to CPU fallbacks for unsupported ops (`aten::angle`). Not a net win.
- **ANE (CoreML Decoder_HAR)**:
  - ~24s utterance: ~1.350 s total → **RTF ≈ 0.057×** (~17x faster than real-time)

**Conclusion**: The ANE provides a dramatic speedup, making it the only viable path for a high-performance, responsive on-device implementation.
